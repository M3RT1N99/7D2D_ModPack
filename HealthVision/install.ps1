...
$target = Join-Path $modsPath "HealthVision"
New-Item -ItemType Directory -Force -Path $target | Out-Null

# Top-level files: copy straight into the target.
foreach ($file in @("ModInfo.xml", "HealthVision.dll")) {
    Copy-Item -LiteralPath (Join-Path $modRoot $file) -Destination (Join-Path $target $file) -Force
}

# Folders: mirror the *contents* into the destination folder (avoids Config\Config nesting).
foreach ($dir in @("Config")) {
